...
同一纲领在爬完三次后转而变宽：一条定理把纯 $n$ 次开方一次性带到所有
次数——$\sqrt[n]{x}=\exp(\ln x/n)$ 满足 $(\sqrt[n]{x})^n=x$ 且是唯一正
解，于是五次开方与一切自然数次的开方不是梯子而是一条陈述。五次扣下的
不是开方，而是混合塔。
主推导中每一条陈述都经机器检查；
...
\item 因此一条形式化陈述同时覆盖所有次数：$n=2,3,5,\dots$。
\end{enumerate}
...
一起，且不需要对 $x$ 单独假设正性——$a>0$ 与 $a^n=x$ 已推出 $x>0$。
...
次数都存在（定理~\ref{thm:anydegree}）；在五次死掉的是\emph{混合}根式
表达式：一般五次 $x^5+ax^4+bx^3+cx^2+dx+e=0$ 不能被任何由域运算与
任意次开方搭成的塔解出（Abel--Ruffini \citep{Abel1824}）；精确陈述这个
边界是纲领的 Galois 半边 \citep{Galois1846}，本文不声称完成它。同样
尚未落码：带 $x^2$ 项的一般三次（平移化为缺项型）与四次（Ferrari，
经三次预解式）。
\end{itemize}
...
一个对比值得点明。纯根式是民主的：对每个次数 $n$，$\mathbb R_+$ 上的
$n$ 次开方都存在、单值、是幂映射的逆（定理~\ref{thm:anydegree}）——
求解器之梯只是混合塔的假象。混合根式塔止于四次、毁于五次
（Abel--Ruffini \citep{Abel1824}、Galois \citep{Galois1846}）；机器在
它能处把对比的两侧都立成定理，在不能处只作引用。
